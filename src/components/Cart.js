...
export default () => {
    const { cart, cartVisible, addToCart, deleteCartItem, increaseCartItem, decreaseCartItem, total } = useCart()
            ...
            {cart.map(item => {
                ...
                return (
                    <div className="container-item">
                        <div onClick={() => deleteCartItem(item.id)} className="delete-cart-item"><FaTimes /></div>
                        ...
                        <div className="item.price">
                            ...
                            <div>
                                <button disabled={item.quantity <= 1} className="product-quantity-button" onClick={() => decreaseCartItem(item.id)}>-</button>
                                <button className="product-quantity-button" onClick={(addToCart) => increaseCartItem(item.id)}>+</button>
                            </div>
                        </div>
                    </div>
                )
